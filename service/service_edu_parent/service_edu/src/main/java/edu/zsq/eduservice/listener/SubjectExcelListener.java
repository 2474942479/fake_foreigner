...
public class SubjectExcelListener extends AnalysisEventListener<SubjectData> {
    ...
    public EduSubject existSubject(EduSubjectService eduSubjectService, String title, String pid) {
        return eduSubjectService.lambdaQuery().eq(EduSubject::getTitle, title).eq(EduSubject::getParentId, pid).last(Constants.LIMIT_ONE).one();
    }
    ...
    public void invoke(SubjectData data, AnalysisContext analysisContext) {
        ...
        if (data == null) {
            throw ExFactory.throwBusiness("excel文件为空");
        }
